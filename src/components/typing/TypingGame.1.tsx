...
import { type ContentHandler } from "../content/TypingGameSource.ts";
...
import { PendingKind } from "../AppState.ts";
import { TypingGameProps } from "./TypingGame.tsx";

export const TypingGame = (props: TypingGameProps) => {
    ...
    const onShuffle = () => {
        ...
        if (props.gameOptions.modeSelected === GameModeKind.timer &&
    ...
    const [timeCounter, setTimeCounter] = createSignal<number | undefined>(
        props.status.content.kind === GameModeKind.timer
            ? props.status.content.time
            : undefined
    );
    ...
    createEffect(
        (typingMetricsState: TypingMetricsState) => typingMetricsState({ event: status() }),
        updateMetrics
    );

    const keyMetrics = createMemo(
        (projection: KeysProjection) => updateKeyProjection({ projection, status: status() }),
        {}
    );
        ...
        <div class="mega" onClick={() => focus()}>
            ...
            <Portal mount={document.getElementById("header-nav-actions-portal")!}>
                <HeaderNavActions
                    ...
                    isGenerated={props.gameOptions.categorySelected.kind === CategoryKind.generation}
                    onPause={() => pause()}
                    onReset={reset}
                    onShuffle={onShuffle}
                    onExit={props.onExit}
